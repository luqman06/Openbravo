...
static Logger log4j = Logger.getLogger(RptPromissoryNoteHeaderData.class);
  ...
  public static RptPromissoryNoteHeaderData[] select(ConnectionProvider connectionProvider, String cDebtPaymenntId, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "          SELECT C_DEBT_PAYMENT_ID, PARTNER_NAME, ADDRESS, POSTAL, REGION, AMOUNT, (CASE WHEN MAX(DOCUMENTNO_VENDOR) IS NOT NULL THEN 'Pago su fra. n��:' ||MAX(DOCUMENTNO_VENDOR) END) AS DOCUMENTNO_VENDOR , (CASE WHEN MAX(DOCUMENTNO_CUSTOMER) IS NOT NULL THEN 'Nuestra fra. n��: '|| MAX(DOCUMENTNO_CUSTOMER) END) AS DOCUMENTNO_CUSTOMER, '' AS DOCUMENTNO, " +
    ...
    ResultSet result;
    Vector<RptPromissoryNoteHeaderData> vector = new Vector<RptPromissoryNoteHeaderData>(0);
  ...
  public static RptPromissoryNoteHeaderData[] selectDebtPayment(ConnectionProvider connectionProvider, String cDebtPaymenntId, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "          C_REGION.NAME AS REGION, C_DEBT_PAYMENT.AMOUNT, (CASE C_INVOICE.ISSOTRX WHEN 'N' THEN 'Pago su fra. n��: '||C_INVOICE.POREFERENCE ELSE 'Nuestra fra. n��: '||C_INVOICE.DOCUMENTNO END) AS DOCUMENTNO, " +
      "          TO_CHAR(now(), 'DD')||' de '||TO_CHAR(now(), 'Month')||' de '||TO_CHAR(now(), 'YYYY') AS DATEINVOICED" +
