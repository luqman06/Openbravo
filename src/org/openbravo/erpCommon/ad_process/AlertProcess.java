...
public class AlertProcess implements Process {
    ...
    try {
      ...
      if (alertRule != null && alertRule.length != 0) {
        ...
        for (int i = 0; i < alertRule.length; i++) {
          processAlert(alertRule[i], connection, adClientId);
        }
      }
    } catch (Exception e) {
  ...
  private static AlertProcessData[] selectAlert(ConnectionProvider connectionProvider,
      String alertRule, String alertRuleId, String clientID) throws ServletException {
    ...
    if (!clientID.equalsIgnoreCase("0"))
    	strSql += " and aaa.ad_client_id='"+clientID+"'";
  ...
  private void processAlert(AlertProcessData alertRule, ConnectionProvider conn, String clientID) throws Exception {
    ...
    if (!alertRule.sql.equals("")) {
      try {
        if (!alertRule.sql.toUpperCase().trim().startsWith("SELECT ")) {
          ...
          alert = selectAlert(conn, alertRule.sql, alertRule.adAlertruleId, clientID);
        }
      } catch (Exception ex) {
    ...
    // build document link prefix
    OBContext.setAdminMode();
    	//get tab id
    	AlertRule alertRule2 = OBDal.getInstance().get(AlertRule.class, alertRule.adAlertruleId);
     	String tabid = "";
    	if (alertRule2.getTab()!=null)
    		tabid = alertRule2.getTab().getId();
    	
    	//get context.url
    	String contexturl = OBPropertiesProvider.getInstance().getOpenbravoProperties().getProperty("context.url");
    OBContext.restorePreviousMode();
    String documentlinkPrefix = contexturl+"/?tabId="+tabid+"&recordId=";
    ...
    if (alert != null && alert.length != 0) {
      ...
      for (int i = 0; i < alert.length; i++) {
        ...
        //add document link at the bottom
        String documentlink = documentlinkPrefix+alert[i].referencekeyId;
        msg.append(System.lineSeparator()).append("Link to document: ").append(documentlink);
        newMsg.append(System.lineSeparator()).append("Link to document: ").append(documentlink);
		...
		docNo = alert[i].recordId;
		refKeyId = alert[i].referencekeyId;
      }
      
      // subject message 
      HashMap<String, StringBuilder> messageByClientOrgForSubject = new HashMap<String, StringBuilder>();
      ...
      for (int i = 0; i < alert.length; i++) {
        ...
        String messageLine = alert[i].description;
      
        newMsg.append(messageLine);
   
        String clientOrg = alert[i].adClientId + CLIENT_ORG_SEPARATOR + alert[i].adOrgId;
        if (messageByClientOrgForSubject.containsKey(clientOrg)) {
        	messageByClientOrgForSubject.get(clientOrg).append(newMsg); 
        } else {
        	messageByClientOrgForSubject.put(clientOrg, newMsg);
        }
          ...
          try {
            ...
            if (mailConfigList.size() > 0) {
             ...
             // Create Subject message
                final StringBuilder SubjectEmail = new StringBuilder();
                  ...
                  if (currentAlertRecipient.getClient().getId().equals(clientAndOrg[0])) {
                    for (RoleOrganization roleOrganization : currentAlertRecipient.getRole()
                      ...
                      if (OBContext.getOBContext().getOrganizationStructureProvider()
                    	  ...
                    	  SubjectEmail.append(messageByClientOrgForSubject.get(currentClientAndOrg)); 
                        break;
                      }
                    }
                  }
                ...
                for (User targetUser : usersList) {
                  ...
                  final String subject = "[OB Alert] " + SubjectEmail; //tiko
				  ...
				  // ############# PUSH NOTIFICATION #############
                  try {
					final String subjectAppAlert = " " + SubjectEmail;
                    String jsonResponse;
                    ...
                    con.setRequestProperty("Authorization",
                        "Basic OWQ3YTUxMDEtZDVhZi00ZjE3LThlYjktMzM0ZWJjNDRkYjI0");
                    con.setRequestMethod("POST");

                    String headingsAppsNotification = "ICS Mobile";

                    if (docNo.substring(0, 2).equals("PR")) {
                      headingsAppsNotification = "Purchase Requisition";
                    } else if (docNo.substring(0, 2).equals("PO")) {
                      headingsAppsNotification = "Purchase Order";
                    } else if (docNo.substring(0, 2).equals("GM")) {
                      headingsAppsNotification = "Goods Movement";
                    }

                    String strJsonBody = "{"
                        + "\"app_id\": \"ccaab3b9-b5b2-480d-ab6b-ef4c65f2d32f\","
                        + "\"filters\": [{\"field\": \"tag\", \"key\": \"MAIL\", \"relation\": \"=\", \"value\": \""
                        + targetUserEmail + "\"}]," + "\"headings\": {\"en\": \""
                        + headingsAppsNotification + "\"}," + "\"contents\": {\"en\": \""
                        + subjectAppAlert + "\"}," + "\"data\": {\"id\": \"" + refKeyId
                        + "\", \"doc_no\": \"" + docNo + "\"}" + "}";
