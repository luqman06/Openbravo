      ...
      for (int k = 0; k < ids.length; k++) {
        ...
        if (data != null) {
          ...
          for (int i = 0; i < data.length; i++) {
        	  ...
        	  //added by zaien: makenetunitpricefixed
        	  String iseditlinenetamt = data[i].iseditlinenetamt;
        	  if (iseditlinenetamt==null || iseditlinenetamt.isEmpty())
        		  iseditlinenetamt="N";
        	  String em_oez_makenetunitpricefixed = data[i].emOezMakenetunitpricefixed;
        	  if (em_oez_makenetunitpricefixed==null || em_oez_makenetunitpricefixed.isEmpty())
        		  em_oez_makenetunitpricefixed="N";
        	  String cancelpricead = data[i].cancelpricead;
        	  if (cancelpricead==null || cancelpricead.isEmpty())
        		  cancelpricead="N";
        	  boolean makenetunitpricefixed = false;
        	  if (iseditlinenetamt.equals("Y") && em_oez_makenetunitpricefixed.equals("Y") && cancelpricead.equals("Y"))
        		  makenetunitpricefixed=true;
            ...
            if (makenetunitpricefixed){
            	OrderLine orderLine = OBDal.getInstance().get(OrderLine.class, data[i].cOrderlineId);
            	BigDecimal actualQtyordered = orderLine.getOrderedQuantity();
            	BigDecimal actualQtyinvoiced = orderLine.getInvoicedQuantity();
            	if (actualQtyinvoiced.equals(BigDecimal.ZERO) && actualQtyordered.toString().equals(data[i].id)){
            		//belum pernah di invoice, dan qty yg AKAN di invoice sama persis dengan yang ada di SO,
            		//sehingga ini adalah copy paste
            		//dan karena makenetunitpricefixed=true, maka jangan hitung ulang line net amount, samakan saja dengan SO
            ...
            try {
              ...
              String strMakenetunitpricefixed="N";
              if (makenetunitpricefixed)
            	  strMakenetunitpricefixed="Y";
              CreateFromInvoiceData.insert(conn, this, strSequence, strKey, vars.getClient(),
                  ...
                  data[i].mInoutlineId.equals("") || data[i].mInoutlineId == null ? data[i].isorder
                      : "N", data[i].cAum, data[i].aumqty, strMakenetunitpricefixed);
