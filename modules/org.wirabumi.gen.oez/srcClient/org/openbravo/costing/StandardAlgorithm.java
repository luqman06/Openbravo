...
public class StandardAlgorithm extends CostingAlgorithm {
	...
	Logger logger = Logger.getLogger(StandardAlgorithm.class);
  ...
  protected BigDecimal getOutgoingTransactionCost() {
    ...
    if (standardCost==null){
    	logger.debug("gagal ketemu standard cost, maka pending. produk: "+transaction.getProduct().getName());
    	transaction.setCostingStatus("P");
    	OBDal.getInstance().save(transaction);
    	return null;
    } else{
    	//standard cost ketemu, jangan biarkan dia tetap di P, kembalikan ke NC, nanti costing server yang akan menjadikannya CC.
