...
public class CostingBackground extends DalBaseProcess implements KillableProcess {
  ...
  protected void doExecute(ProcessBundle bundle) throws Exception {
    ...
    List<String> skippedMTransactionIDs = new ArrayList<>();
    ...
    try {
      ...
      List<String> skipProductID = new ArrayList<>();
      ...
      while (true) {
        ...
        for (String trxId : trxs) {
          ...
          if (skipProductID.contains(productID))
          {
        	  skippedMTransactionIDs.add(transaction.getId());
        	  transaction.setCostingStatus("S");
          ...
          if (log4j.isDebugEnabled())
        	  System.out.println(String.format("[%d] processing transaction [%s] product id [%s]", 
        		  i, 
        		  transaction.getId(),
        		  productID));
          ...
          try 
          {
        	  ...
        	  String message = String.format("failed to calculate product cost [%s] %s", product.getSearchKey(), product.getName());
        	  message += "|-- "+e.getMessage();
        	  logger.logln(message);
        	  log4j.debug(message);
        	  
        	  skipProductID.add(productID);
        	  skippedMTransactionIDs.add(transaction.getId());
      ...
      logger.logln(OBMessageUtils.messageBD("Success"));
      String footerMessage = String.format("Processed %d batch(s) for %s record(s). Failed %s record(s).", batch, counter, failedTrxs); 
      logger.logln(footerMessage);
      ...
      //put skipped product into pending
      for (String skippedTransactionID : skippedMTransactionIDs){
    	  MaterialTransaction transaction = OBDal.getInstance().get(MaterialTransaction.class, skippedTransactionID);
