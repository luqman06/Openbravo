CREATE OR REPLACE FUNCTION public.m_promotion_calculate(p_type character varying, p_doc_id character varying, p_user_id character varying)
 RETURNS void
 LANGUAGE plpgsql
AS $function$ DECLARE 
/*************************************************************************
...
************************************************************************/
  v_date TIMESTAMP;
  v_partner VARCHAR(32); --OBTG:VARCHAR2--
  ...
  v_count NUMERIC;
  v_apply_discount NUMERIC:= 1;

  --TYPE RECORD IS REFCURSOR;
  Cur_Order RECORD;
  Cur_Offer RECORD;
  v_totalamt NUMERIC;
  v_displayedtotalamt NUMERIC;
...
BEGIN
  ...
  for Cur_Order in (select c_orderline_id as id, gross_unit_price, c_tax_id, cancelpricead, 
    ...
    if (p_type = 'O') then
      ...
      if (v_count > 0 and Cur_Order.cancelpricead='N') then
     ...
     select count(*) 
      ...
      loop
        ...
        v_extension_point_id VARCHAR(32) := 'F20050CAA4E1420B8766E1867F7D07CC'; --OBTG:VARCHAR2--
        ...
        BEGIN
          ...
          PERFORM AD_EP_INSTANCE_PARA_INSERT(v_ep_instance, v_extension_point_id, 'Product',
