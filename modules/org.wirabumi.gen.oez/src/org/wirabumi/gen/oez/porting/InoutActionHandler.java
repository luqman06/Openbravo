	...
	public void doRouting(String adWindowId, String adTabId,
		...
		for (String inoutID : recordId){
			...
			if (doc_status_to.equalsIgnoreCase(completeStatus)){
				...
				for (ShipmentInOutLine inOutLineid : inout.getMaterialMgmtShipmentInOutLineList() ) {   

					if (inout.getSalesOrder() == null ) {
						continue;
					}

					BigDecimal orderQuantityGR = inOutLineid.getMovementQuantity(); 
					OrderLine lineGoodReceipt =  inOutLineid.getSalesOrderLine();  

					BigDecimal productOverDelivery = lineGoodReceipt.getProduct().getOezOverDelivery(); 
					if (productOverDelivery == null)
					{  
						productOverDelivery = new BigDecimal (0); // jika over null maka over akan di set menjadi 0 
					}    
					BigDecimal orderQuantityPO = lineGoodReceipt.getOrderedQuantity();
					BigDecimal reservedQuantity = lineGoodReceipt.getReservedQuantity(); 

					BigDecimal bagi100 = BigDecimal.valueOf(100); 
					BigDecimal hasilPoPlusOver =  productOverDelivery.divide(bagi100).multiply(orderQuantityPO).add(orderQuantityPO);
					BigDecimal reserVerif = hasilPoPlusOver.add(reservedQuantity); 

					if (orderQuantityGR.intValue() > reserVerif.intValue())
					{
						throw new OBException("GR (Goods Receipt) quantity is higher than PO (Purchase Order) quantity (with Over delivery tolerance)");  
					}
