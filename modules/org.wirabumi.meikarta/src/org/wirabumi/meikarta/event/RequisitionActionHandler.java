...
public class RequisitionActionHandler extends DocumentRoutingHandlerAction {
	...
	public void doRouting(String adWindowId, String adTabId,
		...
		for (String requisitionID : recordId){
			...
			//cek apakah mengandung barang IT
			if (!requisition.getDocumentStatus().equals("OEZ_REVIEW") && doc_status_to.equals("CO")){
				//bukan reviewed dan akan di-complete, maka cek apakah mengandung barang IT
				boolean mengandungIT = cekApakahMengandungBarangIT(requisitionID);
				if (mengandungIT){
					//document status bukan REVIEWED, tapi mengandung barang IT, maka exception.
					throw new OBException("Terdapat barang IT dalam requisition ini."
							+ " Requisition tidak bisa di-complete sebelum di-review oleh Dept. IT.");
				}
			}
			...
			if (doc_status_to.equals(reactiveStatus))
				EmailUtility.sendEmailReactivedRequsition(requisition);
	...
	private boolean cekApakahMengandungBarangIT(String requisitionID) {
		Requisition requisition = OBDal.getInstance().get(Requisition.class, requisitionID);
		for (RequisitionLine line : requisition.getProcurementRequisitionLineList()){
			String category = line.getProduct().getProductCategory().getName();
			if (category.contains("IT"))
				return true;
		}
		return false;
	}
