...
public class PurchaseRequestPrintService implements PrintService {
	...
	public PurchaseRequestPrintService(String documentID, VariablesSecureApp vars){
		...
		String strReportName = Utility.getPreference(this.vars, "Requisition_ReportTemplate", "800092");
		if (strReportName==null || strReportName.isEmpty() || strReportName.equals(""))
			strReportName = "@basedesign@/org/wirabumi/meikarta/report/purchase_request.jrxml";
	...
	private List<FieldProvider> buildDataSource(Requisition requisition) {
		...
		for (RequisitionLine line : rl){
			i++;
			String lineDescription = line.getProduct().getName();
			if (line.getDescription()!=null)
				lineDescription = lineDescription+System.lineSeparator()+line.getDescription();
	...
	private String buildOutputFileName(String documentID) {
		...
		Requisition requisition = OBDal.getInstance().get(Requisition.class, documentID);
		
		SimpleDateFormat df = new SimpleDateFormat("ddMMyy");
		String strdate = df.format(date);
		String strseqno = formater.format(seqno);
		
		return "PR-"+strdate+strseqno+"-"+requisition.getDocumentNo()+".pdf";
	}
