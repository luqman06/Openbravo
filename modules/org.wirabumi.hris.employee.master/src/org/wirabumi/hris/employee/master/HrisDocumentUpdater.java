...
import org.wirabumi.gen.oez.event.DocumentRoutingHandlerServer;

public class HrisDocumentUpdater extends BaseActionHandler {

  final static Logger log4jDocRouting = Logger.getLogger(HrisDocumentUpdater.class);
  ConnectionProvider connectionProvider = new DalConnectionProvider();

  @Override
  protected JSONObject execute(Map<String, Object> parameters, String content) {
    JSONObject response = new JSONObject();
    String referenceId = "131";
    try {
   ...
   // cek apakah dalam record yang dicentang ada perbedaan docSatus, jika Ya maka exception
      ...
      String doc_status_from=getCurrentStatus(windowId, adTabId, referenceId, recordIdList);
      ...
      DocumentRoutingHandlerAction callHandler = DocumentRoutingHandlerServer.getClassRoutingHandler(windowId, adTabId, doc_status_to, doc_status_from);
      ...
      if (!action.equals("OpenPopupParamater")) {
          ...
          try {
          	OBContext.setAdminMode();
              callHandler.doRouting(windowId, adTabId, doc_status_to, vars, recordIdList);
              String message = Utility.parseTranslation(connectionProvider, vars, vars.getLanguage(),
                  RecordList.length() + " @RowsUpdated@ ");
