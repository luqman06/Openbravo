package org.wirabumi.fifo.process;
...
public class FIFOCosting extends CostingAlgorithm {
	...
	protected BigDecimal getOutgoingTransactionCost() {
		...
		Warehouse warehouse = transaction.getStorageBin().getWarehouse();
		OBCriteria<Costing> costingC = OBDal.getInstance().createCriteria(Costing.class);
		costingC.add(Restrictions.eq(Costing.PROPERTY_PRODUCT, product));
		costingC.add(Restrictions.gt(Costing.PROPERTY_FIFOAVAILABLEQTY, BigDecimal.ZERO));
		costingC.add(Restrictions.eq(Costing.PROPERTY_WAREHOUSE, warehouse));
		...
		for (Costing costing : costingList){
			...
			BigDecimal availableqty = costing.getFifoAvailableqty();
			...
			if (sisa.compareTo(BigDecimal.ZERO)<0){
				...
				costing.setFifoAvailableqty(BigDecimal.ZERO);
			} else {
				usedqty=transactionqty;
				costing.setFifoAvailableqty(sisa);
			}
	...
	private boolean cekQtyCukup(List<Costing> costingList, double trxQty) {
		...
		for (Costing costing : costingList){
			BigDecimal available = costing.getFifoAvailableqty();
			if (available==null)
				continue;
...
//	      return getManufacturingProducedCost();
	...
	protected BigDecimal getShipmentNegativeCost() {
		...
		for (FIFOTransaction ft : mTrans.getFifoFifotransactionList()){
			...
			ftReturn.setCosting(ft.getCosting());
			...
			if (qty.compareTo(mTransQTY)>=0){
				...
				costing.setFifoAvailableqty(costing.getFifoAvailableqty().add(mTransQTY));
				...
				//4. tambahkan available qty sejumlah qty return
				costing.setFifoAvailableqty(costing.getFifoAvailableqty().add(qty));
			}
	...
	protected BigDecimal getReceiptNegativeCost() {
		...
		BigDecimal qty = costing.getFifoAvailableqty();
		costing.setFifoAvailableqty(qty.subtract(qtyReturn));
		OBDal.getInstance().save(costing);
	...
	protected BigDecimal getInventoryIncreaseCost() {
		...
		costing.setCostType("FIFO_FIFO");
		costing.setFifoAvailableqty(qty);
		costing.setQuantity(qty);
		costing.setProduct(product);
		costing.setWarehouse(transaction.getStorageBin().getWarehouse());
