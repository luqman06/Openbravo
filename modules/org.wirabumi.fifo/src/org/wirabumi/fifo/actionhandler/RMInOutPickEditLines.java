...
public class RMInOutPickEditLines extends BaseProcessActionHandler {
  ...
  private void createInOutLines(JSONObject jsonRequest, List<String> idList) throws JSONException {
    ...
    for (long i = 0; i < selectedLines.length(); i++) {
      ...
      //added by zaien: add cancel io id
      OrderLine refOrderLine = orderLine.getSOPOReference();
      if (refOrderLine!=null){
    	  List<ShipmentInOutLine> refInoutLineList = refOrderLine.getMaterialMgmtShipmentInOutLineList();
    	  if (refInoutLineList.size()>0){
    		  ShipmentInOutLine refInoutLine = refInoutLineList.get(0);
    		  newInOutLine.setCanceledInoutLine(refInoutLine);
    	  }
      }
