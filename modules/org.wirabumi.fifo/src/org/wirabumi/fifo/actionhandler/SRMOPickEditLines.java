 ...
 ************************************************************************
 */
package org.wirabumi.fifo.actionhandler;
...
public class SRMOPickEditLines extends BaseProcessActionHandler {
  ...
  private void createOrderLines(JSONObject jsonRequest, List<String> idList) throws JSONException,
    ...
    for (long i = 0; i < selectedLines.length(); i++) {
      ...
      //added by zaien: link return order line to source GS
      String sourceGSLineID = selectedLine.getString("goodsShipmentLine");
      ShipmentInOutLine sourceGSLine = OBDal.getInstance().get(ShipmentInOutLine.class, sourceGSLineID);
      if (sourceGSLine!=null){
    	  OrderLine sourceOrderLine = sourceGSLine.getSalesOrderLine();
    	  newOrderLine.setSOPOReference(sourceOrderLine);
      }
