	...
	public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con, VariablesSecureApp vars)
	    ...
	    for (int i = 0; i < p_lines.length; i++) {
	      ...
	      if (woA.isDisposed()) {
		      ...
		      Account akunAkumulasiDepresiasi = new Account(conn, acAkumulasiDepresiasi.getId());
		      cr = fact.createLine(lineCostcenterBaru, akunAkumulasiDepresiasi, costCurrency.getId(), (akumulasidepresiasi.negate()).toString(),
		          Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
		      if (cr == null) {
		        continue;
		      }
		      ...
		      //ayat jurnal ke-3.
		      log4jDocWorkOrder.debug("CreateFact - akumulasi depresiasi di cost center baru DR: " + akumulasidepresiasi);
		      dr = fact.createLine(lineCostcenterLama, akunAkumulasiDepresiasi, costCurrency.getId(), akumulasidepresiasi.toString(), Fact_Acct_Group_ID,
		          nextSeqNo(SeqNo), DocumentType, conn);
		      if (dr == null) {
		        continue;
		      }
		      ...
		      //ayat jurnal ke-4.
		      log4jDocWorkOrder.debug("CreateFact - harga perolehan di cost center baru DR: " + hargaperolehan);
		      cr = fact.createLine(lineCostcenterLama, akunHargaPerolehan, costCurrency.getId(), (hargaperolehan.negate()).toString(),
		          Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
		      if (cr == null) {
		        continue;
		      }
